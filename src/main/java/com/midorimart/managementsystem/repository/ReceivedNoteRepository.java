...
public interface ReceivedNoteRepository extends JpaRepository<ReceivedNote, Integer>{
    ...
    @Query(value = "select * from Received_note where created_by = ?1", nativeQuery = true)
    List<ReceivedNote> findByUserId(int id, int offset, int limit);

    @Query(value = "select * from Received_note where created_at between ?1 and ?2", nativeQuery = true)
